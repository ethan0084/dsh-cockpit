---
name: dsh-install
description: Install, update, verify, or start DeepSeek Harness with the public Ethan Workbench plugin from npm. Use when a user asks to install DSH, install Ethan Workbench, set up the DSH web profile, replace an old private DSH restore workflow, or verify a public installation. Installs DSH 0.1.0-rc.6 and ethan-workbench 0.1.1 without migrating sessions, settings, projects, credentials, a desktop shell, or patched dependency files.
---
...
1. Run `scripts/install_dsh.sh --check` and report missing prerequisites.
2. Require Node.js 22 or newer and npm. Help the user install a current Node.js LTS release when needed.
3. When the user asks to install or update, run `scripts/install_dsh.sh --install`.
4. Run `scripts/install_dsh.sh --verify` after installation.
5. Start DSH only when requested, using `scripts/install_dsh.sh --launch`. Keep the resulting server process attached unless the user asks for background operation.
...
- If verification reports missing Workbench entries, rerun `--install`; the operation is idempotent.
